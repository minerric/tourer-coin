...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='tourer.conf'
CONFIGFOLDER='/root/.tourer'
COIN_DAEMON='tourerd'
COIN_CLI='tourer-cli'
COIN_PATH='/usr/local/bin/'
COIN_TGZ=`curl -s https://api.github.com/tourercoin/tourer/releases/latest | grep "browser_download_url.*Linux\\.zip" | cut -d : -f 2,3 | tr -d \" | xargs`
