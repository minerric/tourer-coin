...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Tourer client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Tourer"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Tourer"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
