    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of TOURER earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 TOURER");
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("TOURER")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair TOURER
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(204, 41, 82));
	set1->setColor(QColor(112, 36, 61));
